...
jsPsych.plugins["RDK"] = (function() {
	...
	plugin.trial = function(display_element, trial) {
		...
		//trial.aperture_center_x = assignParameterValue(trial.aperture_center_x, window.innerWidth/2 - (0.045*window.innerWidth/2));
		//trial.aperture_center_y = assignParameterValue(trial.aperture_center_y, window.innerHeight/2);
		trial.aperture_center_x = window.innerWidth/2- (0.045*window.innerWidth/2);
    ...
    document.body.style.overflow = 'hidden';
		...
		function end_trial() {
			...
			var trial_data = {
				...
				"correct": correctOrNot()==true ? 1 : 0, //If the subject response was correct
			...
			if(typeof trial.correct_choice !== 'undefined'){
				...
				//Else compare the char with the response key
					...
					if(typeof trial.correct_choice === 'string' || trial.correct_choice instanceof String){
						//console.log("d_press "+response.key);
            //console.log("d_correct "+trial.correct_choice.toUpperCase().charCodeAt(0));
