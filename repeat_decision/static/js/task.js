...
word_list = nestArray(library_list, 9)
...
function runExperiment() {
  ...
  var n3 = 3; //multiple of three. If we set 6 words per trial, then n3 should be 2.
  ...
  var welcome_block = {
    ...
    message: `<p>Welcome to the experiment.</p>
                ...
                <p>The bonus can be corrected if you decide to repeat your decision.</p>
  ...
  var instruction1 = {
    type: "html-keyboard-response",
    choices: [' '],
    stimulus: "<p>In the first part, on each trial, you will see an array of moving dots." +
      ...
      "<div style='float: center;'><img src='/repeat_decision/static/images/first_phase_simple.png'></img></div>" +
  ...
  var instruction2 = {
    ...
    stimulus: "<h3>(Second Phase)</h3>" +
      "<p>One of two different events will show up (with a randomized order): a <strong>Memory Recall</strong> or a <strong>Repeat Decision</strong>.</p>" +
      "<p>Possibility #1 <strong>Memory Recall</strong>: You will now see each word again. When you see the word, select which direction you chose (LEFT or RIGHT) for the associated dot array.</p>" +
      "<p>After you select a direction, you will be asked about how accurate you believe your memory is (LOW or HIGH).</p>" +
      "<p>Possibility #2 <strong>Repeat Decision</strong>: You will also be asked if you wish to revisit the array of dots (NO or YES).</p>" +
      "<p>This will allow you to revisit the dot array decision to possibly revise your answer.</p>" +
      ...
      "<div style='float: center;'><img src='/repeat_decision/static/images/second_phase_simple.png'></img></div>" +
  ...
  var instruction3 = {
    ...
    stimulus: "<p>You will use your left index finger to press the <strong>Q</strong> key to specify LEFT, and your right index finger to press the <strong>P</strong> key to specify RIGHT.</p>" +
      ...
      "<p>Press the SPACEBAR when you are ready to proceed.</p>" +
      "<div style='float: center;'><img src='/repeat_decision/static/images/instruction_simple.png'></img></div>"
  }
  ...
  var instruction4 = {
    ...
    stimulus: "<p>You will get a <b>bonus</b> depending on the <b>number of correct responses</b> (correctly judging the direction of dots when seeing a dot array, and correctly remembering your decision).</p>" +
      "<p>It is important to try to do both correctly -- to make correct decisions, and to remember them. Repeating a decision and correcting it will allow you to increase your bonus.</p>" +
      "<p></p>" +
      "<p>Press the SPACEBAR when you are ready to proceed.</p>"
  }
  ...
  //  type: 'survey-text',
  //  questions: [
  //    {prompt: 'Was there anything confusing or that you would change about the task?',
  //    columns: 80,
  //    rows: 10}
  //  ],
  ...
  //      cache: true,
  //      url: '',
  //      data: {'subject_id': data.subject_id, 'study_id': data.study_id, 'session_id': data.session_id, 'comment': jsPsych.data.get().last(1).values()[0].response['Q0']}, // Include other variables you want to send to Django
  //    });
  //  }
  //}
  ...
  var phase_2_instruction = {
    ...
    stimulus: "<h3>(Second Phase)</h3>" +
      "<p>One of two different events will show up (with a randomized order): a <strong>Memory Recall</strong> or a <strong>Repeat Decision</strong>.</p>" +
      "<p><strong>Possibility 1 Memory Recall</strong>: You will now see each word again. When you see the word, select which direction you chose (LEFT or RIGHT) for the associated dot array.</p>" +
      "<p>After you select a direction, you will be asked about how accurate you believe your memory is (LOW or HIGH).</p>" +
      "<p><strong>Possibility 2 Repeat Decision</strong>: You will also be asked if you wish to revisit the array of dots (NO or YES).</p>" +
      "<p>This will allow you to revisit the dot array decision to possibly revise your answer.</p>" +
      "<p>Remember, use your left index finger to press the <strong>Q</strong> key to specify LEFT, and your right index finger to press the <strong>P</strong> key to specify RIGHT.</p>" +
      "<div style='float: center;'><img src='/repeat_decision/static/images/second_phase_simple.png'></img></div>" +
      "<p>Press the SPACEBAR to continue.</p>"
  ...
  var demo_list = jsPsych.randomization.shuffle(["terrain", "joker", "splendid", "gorgeous", "elephant", "pencil", "track", "apple", "suit"]);
  ...
  for (var trial_index = 0; trial_index < demo_list.length; trial_index++) {
    ...
    var demo_phase_1_word = {
      type: "html-keyboard-response",
      stimulus: '<p style="font-size:48px;color:red;">' + stimulus + '</p>',
      choices: jsPsych.NO_KEYS,
      trial_duration: 2000,
    };
  ...
  //Index for 1/2 repeat and 1/2 memory
  ////
  var order_index = [0, 1];
  var randomize = jsPsych.randomization.repeat(order_index, (demo_list.length / 2));
  ...
  for (var trial_index = 0; trial_index < demo_list.length; trial_index++) {
    ...
    var demo_phase_2_repeat = {
      type: "html-keyboard-response",
      stimulus: '<p style="font-size:48px;color:red;">' + stimulus + '</p>' +
        '<p style="font-size:48px; ">Do you want to revist your decision?</p>',
      word: stimulus,
      choices: ["q", "p"],
    ...
    var demo_phase_2_word = {
      ...
      stimulus: '<p style="font-size:48px;color:red;">' + stimulus + '</p>',
      word: stimulus,
      prompt: '<br /><br /><br /><img src="/repeat_decision/static/images/reminder_simple.png" />',
      choices: ['q', 'p'],
    }
    ...
    var demo_phase_2_confidence = {
      type: "html-keyboard-response",
      stimulus: '<p style="font-size:48px;color:red;">' + stimulus + '</p>' +
        '<p style="font-size:48px; ">How certain are you of your memory?</p>',
      choices: ["q", "p"],
      prompt: '<p style="font-size:48px; "><br><br>Low(Q)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;High(P)</p>',
    }
    ...
    //#SEAN RANDOMIZATION for 1/2 word and 1/2 repeat trials
    //console.log(trial_index)
    if (randomize[trial_index] == 0) {
      timeline.push(fixation, demo_phase_2_word, blank, demo_phase_2_confidence);
    } else {
      timeline.push(fixation, demo_phase_2_repeat, demo_if_repeat_node);
    }
  ...
  var randomize = jsPsych.randomization.repeat(order_index, (word_list.flat().length / 2));
  ...
  for (var block_index = 0; block_index < word_list.length; block_index++) {
    ...
    for (var trial_index = 0; trial_index < word_list[0].length; trial_index++) {
      ...
      //console.log(((block_index)*word_list[0].length)+(trial_index+1));
      temp_index = ((block_index) * word_list[0].length) + (trial_index + 1);
      ...
      var phase_1_dots = {
        ...
        coherence: current_coherence,
        trial_duration: -1,
        data: { subject_id: subject_id, study_id: study_id, session_id: session_id, temp_index: temp_index, event_type: "first", block_type: block_type, block_num: block_index + 1, trial_label: trial_index + 1, word: stimulus },
        on_finish: function (data) {
          ...
          function myAjaxRequest() {
            $.ajax({
              type: 'GET',
              cache: true,
              url: '/repeat_decision/task/save_phaseone_data',
              data: {
                'subject_id': data.subject_id, 'study_id': data.study_id, 'session_id': data.session_id, 'index': data.temp_index, 'event_type': 'first', 'block_type': data.block_type, 'block': data.block_num, 'trial_label': data.trial_label,
                'coherence': data.coherence, 'dot_correct': data.correct, 'direction': data.coherent_direction, 'key_press': data.key_press,
                'dot_rt': data.rt, 'word': data.word
              }, // Include other variables you want to send to Django
              error: function () {
                setTimeout(() => {
                  myAjaxRequest()
      ...
      var phase_1_confidence = {
        ...
        choices: ["q", "p"],
        prompt: '<p style="font-size:48px; "><br><br>Low(Q)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;High(P)</p>',
        data: { subject_id: subject_id, study_id: study_id, session_id: session_id, temp_index: temp_index },
        on_finish: function (data) {
          ...
          function myAjaxRequest() {
            $.ajax({
              type: 'GET',
              cache: true,
              url: '/repeat_decision/task/save_phaseoneconfidence_data',
              data: { 'subject_id': data.subject_id, 'study_id': data.study_id, 'session_id': data.session_id, 'index': data.temp_index, 'one_confidence': jsPsych.data.get().last(1).values()[0].key_press }, // Include other variables you want to send to Django
              error: function () {
                setTimeout(() => {
                  myAjaxRequest()
                }, 5000) // if there was an error, wait 5 seconds and re-run the function
    ...
    for (var trial_index = 0; trial_index < word_list[0].length; trial_index++) {
      ...
      var phase_2_word = {
        ...
        choices: ['p', 'q'],
        //trial_duration: 2000,
        data: { subject_id: subject_id, study_id: study_id, session_id: session_id, temp_index: temp_index, block_type: block_type, block_num: block_index + 1, trial_label: trial_index + 1, trial_num_d: word_list[0].length * block_index + trial_index + 1, word: stimulus },
        on_finish: function (data) {
          ...
          function myAjaxRequest() {
            $.ajax({
              type: 'GET',
              cache: true,
              url: '/repeat_decision/task/save_phasetworepeat_data',
              data: {
                'subject_id': data.subject_id, 'study_id': data.study_id, 'session_id': data.session_id, 'index': data.temp_index, 'repeat': -9999, 'repeat_rt': -9999,
                'word': data.word, 'word_rt': data.word_rt, 'word_correct': data.word_correct
              }, // Include other variables you want to send to Django
              error: function () {
                setTimeout(() => {
                  myAjaxRequest()
        ...
        on_finish: function (data) {
          ...
          function myAjaxRequest() {
            $.ajax({
              type: 'GET',
              cache: true,
              url: '/repeat_decision/task/save_phasetwo_data',
              data: {
                'subject_id': data.subject_id, 'study_id': data.study_id, 'session_id': data.session_id, 'index': data.temp_index, 'event_type': 'first', 'block_type': data.block_type, 'block': data.block_num, 'trial_label': data.trial_label,
                'coherence': data.coherence, 'dot_correct': data.correct, 'direction': data.coherent_direction, 'key_press': data.key_press,
                'dot_rt': data.rt, 'word': data.word
      ...
      var phase_2_repeat = {
        ...
        choices: ["q", "p"],
        prompt: '<p style="font-size:48px; "><br><br>No(Q)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Yes(P)</p>',
        data: { subject_id: subject_id, study_id: study_id, session_id: session_id, temp_index: temp_index, word: stimulus, trial_num_d: word_list[0].length * block_index + trial_index + 1 },
        on_finish: function (data) {
          var word_rt = jsPsych.data.get().filter({ trial_num: data.trial_num_d }).select('word_rt').values[0];
          var word_correct = jsPsych.data.get().filter({ trial_num: data.trial_num_d }).select('word_correct').values[0];
          function myAjaxRequest() {
            $.ajax({
              ...
              url: '/repeat_decision/task/save_phasetworepeat_data',
              data: {
                'subject_id': data.subject_id, 'study_id': data.study_id, 'session_id': data.session_id, 'index': data.temp_index, 'repeat': jsPsych.data.get().last(1).values()[0].key_press, 'repeat_rt': jsPsych.data.get().last(1).values()[0].rt,
                'word': data.word, 'word_rt': -9999, 'word_correct': -9999
              }, // Include other variables you want to send to Django
              error: function () {
                setTimeout(() => {
                  myAjaxRequest()
        ...
        on_finish: function (data) {
          function myAjaxRequest() {
            $.ajax({
              type: 'GET',
              cache: true,
              url: '/repeat_decision/task/save_phasetwoconfidence_data',
              data: { 'subject_id': data.subject_id, 'study_id': data.study_id, 'session_id': data.session_id, 'index': data.temp_index, 'two_confidence': jsPsych.data.get().last(1).values()[0].key_press }, // Include other variables you want to send to Django
              error: function () {
                setTimeout(() => {
                  myAjaxRequest()
                }, 5000) // if there was an error, wait 5 seconds and re-run the function
      ...
      //console.log(randomize.filter(v => v === 0).length)
      //console.log(randomize.filter(v => v === 1).length)
      if (randomize[temp_index] == 0) {
        timeline.push(fixation, phase_2_word, blank, phase_2_confidence);
      } else {
        timeline.push(fixation, phase_2_repeat, if_repeat_node);
      }
    }
  }
        ...
        if (i != 31) {
          ...
          function myAjaxRequest() {
            $.ajax({
              type: 'GET',
              cache: true,
              url: '/repeat_decision/task/save_ques_data',
  ...
  <p><a href="https://umdsurvey.umd.edu/jfe/form/SV_0uCvdZpV7OasOj4">Click here to return to Prolific and complete the study</a>.</p>`,
    choices: jsPsych.NO_KEYS,
    on_start: function () {
    }
  }
